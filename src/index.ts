...
function colorPalette (input: any, options: any, callback: any) {
...
function paletteFromBitmap (filename: string, options: any, callback: any) {
    if (!callback) {
        callback = options
        options = {
            type: undefined,
    ...
    getPixels(filename, options.type, function (err: any, pixels: any) {
        if (err) return callback(err)
        const palette = getRgbaPalette(pixels.data, options.count).map(function (rgba: any) {
            return chroma(rgba)
        })
